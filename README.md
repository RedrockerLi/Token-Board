...
- **账户类型**:`api`(按量计费)或 `plan`(订阅套餐)。plan 按每把上游密钥的订阅周期收费,与调用量无关;调用仍会记录一笔 api 口径的**虚拟消费**,用于衡量套餐价值。密钥可设置订阅起始日,周期、价格变更与取消判断统一使用 UTC+0。
- **并发限额**:同一账户同时进行中的请求数上限,留空 = 不限。超限的请求立即返回 HTTP 429;在聚合链里则自动切到下一个账户
- **plan 冷却**:plan 的单把上游密钥被限流(HTTP 429)后自动冷却 **5 小时**,同账户的其他密钥仍可立即接管
