...
- **plan 冷却**:plan 的单把上游密钥只有收到**真正的配额耗尽错误**才会自动冷却 **5 小时**——即上游返回 `429` 且错误体是 `{"type":"error","error":{"type":"GoUsageLimitError",…}}`(如 opencode.ai 的"5 小时/每周使用限额")。普通的瞬时 `429`(限流/过载)只做几秒到几分钟的退避,不会锁死密钥;成功调用即清除退避。同账户的其他密钥在任何冷却下都仍可立即接管
