...
- 智能体管理:独立维护订阅与软件来源,通过统一 IR 导入 34 种本地 agent 用量,一个订阅同一时点只绑定一个 Agent,绑定关系可解绑或改绑,实现参考[vibe-usage](https://github.com/vibe-cafe/vibe-usage)
...
- **上游账户类型**只有 `api`(按量计费)和 `plan`(订阅套餐)。plan 按每把上游密钥的订阅周期收费,与调用量无关;调用仍会记录一笔 api 口径的**虚拟消费**,用于衡量套餐价值。密钥可设置订阅起始日,周期、价格变更与取消判断统一使用 UTC+0。Codex 等智能体不属于上游账户,请在**智能体管理**中分别维护订阅和软件来源。
- **并发限额**:同一账户同时进行中的请求数上限,留空 = 不限。显式并发槽耗尽时返回 HTTP 503 + `Retry-After: 1`;在聚合链里则自动切到下一个账户
- **plan 冷却**:plan 的单把上游密钥只有收到**真正的配额耗尽错误**才会自动冷却 **5 小时**——即上游返回 `429` 且错误体是 `{"type":"error","error":{"type":"GoUsageLimitError",…}}`(如 opencode.ai 的"5 小时/每周使用限额")。普通的 `401/403/429/5xx` 和代理超时不会被记忆，下一次请求仍会重新尝试该密钥。同账户的其他密钥在真实配额冷却下仍可立即接管。冷却状态只存在内存中(重启即清),且冷却期内代理会**每隔 1 小时主动探测一次上游**的 `GET /models`:只要上游恢复返回 2xx,该密钥的冷却会提前解除、重新进入候选池,不必等满 5 小时(探测不产生任何请求日志、不占并发)。探测间隔默认 1h,可用环境变量 `TB_COOLDOWN_PROBE_SECS` 覆盖(单位秒)
...
3. 同一模型可以配多个上游账户,请求从上到下依次尝试:当前账户达并发上限或处于真实配额冷却时自动换下一个;上游返回 429 / 5xx 且尚未向客户端发任何数据时,本次请求立即回退;全部真实配额冷却时返回 429,本地并发容量不足时返回 503
...
- 可配置多个输入长度条件档:按完整输入 `prompt_tokens` 的十进制 K/M 门槛选择价格；未命中用基本价，条件档未填写的价格字段继承基本价
- 给每个定价配置**时段倍率**(峰谷定价):按「当日几点到几点」配一个倍率,比如夜间 1.2 倍。时段按电脑所在时区输入,跨午夜用起止倒置表示;保存后实际峰谷区间不随电脑移动而改变
- 在模型定价页拖动调整匹配优先级:同一请求命中多个 pattern 时,排在上面的生效
- 写入请求时使用当时的当前定价、时段倍率和汇率并固化费用；改价、调档、换序都只影响之后写入的新请求

消费口径的说明见 [doc/billing-pricing.md](doc/billing-pricing.md)。

## 仪表板使用

侧边栏页面按“数据面板 / 智能体管理 / 代理管理 / 通用设置”分组,时间显示统一为电脑当前时区:

| 页面 | 用途 |
|------|------|
| 用量仪表板 | 总 Token / 请求数 / 消费概览、按近期用量加权的模型占比饼图、Token 类型分布、按模型堆叠的 WebGL 3D 每日用量日历。模型分布按当前自然月向前每月乘以 0.9 加权，并隐藏加权占比不严格大于 1% 的模型；日历仍展示所有真实 Token 用量，隐藏模型在图例中合并为 `Other` |
...
| 模型定价 | 单价与峰谷时段编辑器（通用设置） |
| 请求日志 | 简单分页请求明细(不提供筛选) |
| 性能监控 | 当前并发、RPM、成功率、平均延迟、P50/P95/P99、输出速度分布(P50/P95/P99)、每分钟请求数、各上游成功率、各模型延迟与 TTFT/速度采样,15 秒自动刷新 |
| 设置 | 代理超时配置(按客户端线格式分组)与 WebDAV 同步设置（通用设置） |
| 智能体管理 | 独立维护订阅与软件来源;支持注册表中的本地 agent 用量导入,一个订阅同一时点只绑定一个 Agent,绑定关系可解绑或改绑 |

### 数据看板与代理导出

用量数据来自代理转发和已配置的智能体软件:消费报告页点「导出数据」,未同步的用量按写时固化的 `request_log` 增量聚合写入存档数据库(dashboard.db),已入库金额不受之后改价影响。智能体解析器位于 `app/services/agent_usage/adapters/`，每个 agent 一个文件，统一输出 `UsageEvent`；各来源的本地路径以 `/api/proxy/agent-types` 返回的注册表为准。项目和 session_id 只在导入过程的内存中使用,不再写入数据库或前端。CSV 导入已弃用。

仪表盘中的用户删除只清理已经归档的历史用量和费用，不影响源库中的账号、请求日志或已冻结账单；该账号后续产生的新用量仍可通过「导出数据」写回仪表盘，但已经成功导出的冻结账单不会再次写回。已删除的旧请求不会因再次导出而自动重放。

### WebDAV 同步

多台电脑共用代理时,用 WebDAV 同步配置与聚合用量:

1. **通用设置 → 设置** → 填 WebDAV 服务器地址、账号、密码 → **测试连接** → **保存配置**
...
运行时数据都在 `data/`(已 gitignore):`token-board.db`(代理配置与本机运行库)、`dashboard.db`(用量+总价存档库)。两个库的 schema 由版本化迁移管理,升级规则见 [doc/database-migrations.md](doc/database-migrations.md)。
...
**同一个模型为什么有时不走我想要的账户?**
多账户聚合时,回退顺序是 真实配额冷却跳过 → 并发满跳过 → 429/5xx 回退。普通失败不会把 key 记入跨请求冷却。想固定走某个账户,就把它排在链最前,且只给这个模型配它。
...
**上游长时间没回,连接被断开?**
这是代理的上游读超时。超时按客户端线格式分三档配置(流式首字节 / 流式静默 / 非流式,默认值及说明见**通用设置 → 设置**),改完即时生效。超时后代理会按客户端格式发一个明确的超时错误(504,`timeout_error`),而不是静默断连;非流式的聚合请求会先尝试下一个账户,全部超时才返回 504。SDK 通常会按这个错误重试。

**流式请求为什么有时会等很久然后才失败?**
如果上游在**已经输出了首 token**之后停滞(部分 plan 上游配额耗尽时的表现——不再返回 429,而是接受请求、吐出一点内容、然后卡住),代理会等到「流式静默超时」(`streaming_idle_timeout`,默认 120s)才以 504 终止。因为内容已经写给了客户端,单请求内**不能再切换到下一个账户**(已提交的流式数据无法撤回),这是流式协议的固有边界。可在通用设置里调低静默超时缩短等待;配额恢复后该上游会重新进入候选链。

**想把账单明细带出去?**
明细在本地 `data/token-board.db` 的 `request_log` 表,用 sqlite 工具直接查;云端同步的只是聚合结果。
...
| [doc/upstream-concepts.md](doc/upstream-concepts.md) | 上游账户模型：api/plan 类型、单上游多密钥、聚合上游，及智能体管理的边界 |
