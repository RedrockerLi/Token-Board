...
- 智能体管理:独立维护订阅与软件来源,通过统一 IR 导入 28 种本地 agent 用量,一个订阅同一时点只绑定一个 Agent,绑定关系可解绑或改绑
...
用量数据来自代理转发和已配置的智能体软件:消费报告页点「导出数据」,未同步的用量按写时固化的 `request_log` 增量聚合写入存档数据库(dashboard.db),已入库金额不受之后改价影响。智能体解析器位于 `app/services/agent_usage/adapters/`，每个 agent 一个文件，统一输出 `UsageEvent`；各来源的本地路径以 `/api/proxy/agent-types` 返回的注册表为准。项目和 session_id 只保存在本机 `token-board.db` 的请求日志中,暂不在前端展示。CSV 导入已弃用。
