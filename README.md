...
- 智能体管理:独立维护订阅与软件来源,支持 Codex / OpenCode 用量导入,并可将订阅绑定到多个软件
...
| 智能体管理 | 独立维护订阅与软件来源;支持 Codex / OpenCode 用量导入,订阅可多对多绑定软件 |

### 数据看板与代理导出

用量数据来自代理转发和已配置的智能体软件:消费报告页点「导出数据」,未同步的用量按写时固化的 `request_log` 增量聚合写入存档数据库(dashboard.db),已入库金额不受之后改价影响。Codex 扫描 `~/.codex/sessions/` 与 `~/.codex/archived_sessions/`,OpenCode 读取其 `opencode.db`;项目和 session_id 只保存在本机 `token-board.db` 的请求日志中,暂不在前端展示。CSV 导入已弃用。
...
**Codex 用量为什么没有出现?**
确认“智能体管理 → 软件管理”中已启用对应的软件来源,并检查数据目录是否指向实际的 Codex Home 或 OpenCode 数据目录。导入器在看板启动、每 30 分钟以及打开看板时运行；导入后的本机请求日志需要在消费报告中点击“导出数据”才会进入 dashboard 存档。订阅绑定需要在软件编辑页手动维护,不会自动推断。
