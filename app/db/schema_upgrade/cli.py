...
from .coordinator import UpgradeError, ensure_local_databases
...
def _latest_recovery_paths(data_dir: str | Path) -> tuple[list[str], list[str]]:
    ...
    ``_transition_pair_impl`` writes ``auto-v0-to-v1-{stamp}.manifest.json``
    and ``auto-v0-to-v1-{stamp}.backup/`` beside the proxy database. On a
    failed upgrade those are the resumable/rollback artifacts a user needs
    to know about, so surface them in the error output.
    """
    root = Path(data_dir)
    manifests = sorted(root.glob("auto-v0-to-v1-*.manifest.json"))
    backups = sorted(root.glob("auto-v0-to-v1-*.backup"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--proxy-db", required=True)
    ...
    try:
        result = ensure_local_databases(
            args.proxy_db, args.dashboard_db, args.schema_dir, args.timezone)
    except (UpgradeError, OSError, ValueError) as exc:
        print(f"schema upgrade failed: {exc}", file=sys.stderr)
        manifests, backups = _latest_recovery_paths(Path(args.proxy_db).parent)
        if manifests:
