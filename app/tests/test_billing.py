...
class BillingTest(AppDatabaseTestCase):
    ...
    def test_recurring_charge_requires_historical_fx_and_is_idempotent(self) -> None:
        db = self.proxy_database()
        account_id = db.create_account({
            "name": "usd-plan", "account_type": "plan", "currency": "USD",
            "monthly_price": 10, "valid_from": "2026-08-01",
        ...
        with sqlite3.connect(self.proxy_path) as conn:
            row = conn.execute(
                "SELECT normalized_recurring_cost,fx_rate_date FROM "
        ...
        with sqlite3.connect(self.proxy_path) as conn:
            rows = conn.execute(
                "SELECT normalized_recurring_cost,base_currency,fx_rate_date "
