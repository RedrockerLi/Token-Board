...
| `bash start.sh --debug` | 根目录入口:暂停 systemd 并在终端输出上下游完整 debug；Ctrl+C 后恢复原服务状态 |
...
- `dashboard.js`:用量仪表板(按月衰减的模型分布与 3D 日历)。模型分布按当前自然月向前每月乘以 `0.9`，以全部模型的加权 Token 总量为分母，严格大于 `1%` 的模型按加权顺序展示；日历使用所有模型的真实 Token 用量，未进入饼图的模型使用中性灰并在图例中合并为 `Other`，悬浮卡片仍展开原始模型名。
