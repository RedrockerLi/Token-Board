...
schema 的单一来源是 `schema/` 目录下的版本化迁移文件，见
[database-migrations.md](database-migrations.md)。当前 baseline 位于
`schema/proxy/v1/1-0_baseline.sql` 与 `schema/dashboard/v1/1-0_baseline.sql`；
V0 文件按原内容归档在各自 `v0/`。C++ 与 Python runner 共用 flock、
`schema_version`、checksum 与 `user_version` 镜像协议。
...
├── schema/proxy/v0,v1/     Proxy 历史迁移与当前 V1
├── schema/dashboard/v0,v1/ Dashboard 历史迁移与当前 V1
├── schema/transitions/     不兼容 Major 的离线迁移工具
