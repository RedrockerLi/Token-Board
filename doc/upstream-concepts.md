# 上游账户模型：类型(api/plan/agent)、单上游多密钥与聚合上游
...
```
 ...
 一把钥匙,绑定一个               (聚合? 按模型二次解析)          api / plan / agent
 上游账户(或聚合)                → 候选链(账户×密钥)            is_aggregate=0|1
                                并发闸门 → 转发 → 记账          upstream_keys(多把密钥)
```

- **账户(account)** 是唯一身份，`id` 永不复用，删除是软删(`deleted_at`)。
- **账户类型 `account_type`**：`api`(按量)、`plan`(订阅套餐)、`agent`(Agent 订阅，如 Codex)。它决定**怎么计费**、**能不能路由**、**用量从哪来**。
...
| 是否持有上游密钥 | 是 | 是（**每把密钥 = 一个订阅**） | 否（`create_account` 明确不为 agent 建 key） |
| 是否可被本地密钥路由 | 是 | 是 | **否**（本地密钥创建被拒；路由快照也跳过 agent） |
| 调用量来源 | 代理转发自动记账 | 代理转发自动记账 | **后台导入**（目前只支持 Codex 会话文件） |
| `request_log.api_cost` 含义 | 真实账单 | 虚拟/理论消费(套餐省下的钱) | 虚拟/理论消费 |
| 真实成本存档 | 进 `cost_entry` | 进 `proxy_plan_summary.subscription_cost` | 同 plan（`key_masked='subscription'`） |
| 429 处理 | 瞬时退避(5s/30s/2min) | 每把密钥**冷却 5 小时** | 无路由，不涉及 |
...
一个真实上游账户（api 或 plan）可以在 `upstream_keys` 表挂多把上游 Key（迁移 `0007`，[schema/proxy/0007_multi_key.sql](schema/proxy/0007_multi_key.sql)）。这把真实 Key 是**本地机密**，绝不上云；云端只同步 masked 元数据（`upstream_keys_cloud`）用于跨机账单关联。
...
| `api_cost` | 写时固化的 api 等价价（api=真实账单，plan/agent=虚拟消费），USD 定价按当日汇率换算为 CNY |
| `status_code` / `duration_ms` | 结局 |
| `ttft_ms` / `generation_ms` / `output_tps` | 性能观测 |
| `event_id` / `cost_frozen` | agent 导入幂等标识 / 冻结计价标记 |

写时计价的固化：代理自身走 C++ `snapshot_request_cost` 快照（`cost_frozen=1`），agent 导入与兜底走 `tr_request_log_insert` 触发器（`cost_frozen=0`，[schema/proxy/0014_trigger_fx.sql](schema/proxy/0014_trigger_fx.sql)）。改价不回溯。
...
| 疑问 | 答案 |
|------|------|
| 聚合账户和 plan/api 是一类吗？ | 不是。聚合是路由分组（`is_aggregate=1`），plan/api/agent 是计费类型（`account_type`）。聚合永远不入账。 |
| agent 账户为什么发不了请求？ | 它没有上游密钥、被路由快照排除、本地密钥也不让绑它。它只作为 Codex 会话用量导入的目标。 |
