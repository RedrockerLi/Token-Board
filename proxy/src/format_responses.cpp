    ...
    bool emit(const StreamEvent &ev, const Sink &sink) override {
        switch (ev.type) {
            ...
            case StreamEventType::ContentTextDelta:
                ...
                if (!text_started_.count(ev.index) &&
                    !emit_text_item_start(sink, ev.index)) return false;
                {
                    json d;
                    d["type"] = "response.output_text.delta";
                    d["item_id"] = item_id(ev.index);
                    d["output_index"] = ev.index;
                    d["content_index"] = 0;
                    d["delta"] = ev.text;
                    if (!sink("data: " + d.dump() + "\n\n")) return false;
                }
                text_[ev.index] += ev.text;
                return true;
            case StreamEventType::ContentThinkingDelta:
                if (!started_ && !emit_response_created(sink)) return false;
                if (!text_started_.count(ev.index) &&
                    !emit_reasoning_item_start(sink, ev.index)) return false;
                {
                    json d;
            ...
            case StreamEventType::ToolCallStart:
                if (!started_ && !emit_response_created(sink)) return false;
                {
                    ...
                    d["output_index"] = ev.index;
                    d["item"] = item;
                    tools_[ev.index] = {ev.text, ev.arguments, ""};
                    if (!sink("data: " + d.dump() + "\n\n")) return false;
                }
                return true;
            case StreamEventType::ToolCallArgumentDelta:
                {
                    ...
                    tools_[ev.index].arguments += ev.arguments;
            ...
            case StreamEventType::ToolCallDone:
                {
                    ...
                    item["type"] = "function_call";
                    item["id"] = item_id(ev.index);
                    item["call_id"] = tools_[ev.index].call_id;
                    item["name"] = tools_[ev.index].name;
                    item["arguments"] = ev.arguments.empty() ? tools_[ev.index].arguments : ev.arguments;
                    item["status"] = "completed";
                    json d;
                    d["type"] = "response.output_item.done";
                    d["output_index"] = ev.index;
                    d["item"] = item;
                    tools_[ev.index].arguments = item["arguments"];
                    return sink("data: " + d.dump() + "\n\n");
                }
...
private:
    ...
    Usage last_usage_;
    std::map<int, std::string> text_;       // index → accumulated text
    std::map<int, ToolItem> tools_;         // index → tool item
    std::map<int, int> item_kind_;          // 0=text/reasoning, 1=tool
    std::set<int> text_started_;
    ...
    json build_output() {
        json output = json::array();
        std::set<int> idxs;
        for (auto &kv : text_) idxs.insert(kv.first);
        for (auto &kv : tools_) idxs.insert(kv.first);
        for (int idx : idxs) {
            if (tools_.count(idx)) {
                json item;
                item["id"] = item_id(idx);
                ...
                item["status"] = "completed";
                output.push_back(std::move(item));
            } else if (item_kind_.count(idx) && item_kind_[idx] == 0) {
                json item;
                item["id"] = item_id(idx);
                item["type"] = "message";
                item["status"] = "completed";
