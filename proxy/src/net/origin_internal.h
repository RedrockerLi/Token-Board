...
inline WatchSocket duplicate_socket(WatchSocket sock) {
#ifdef _WIN32
...
inline void close_socket_copy(WatchSocket sock) {
...
inline void shutdown_socket_copy(WatchSocket sock) {
    if (sock == INVALID_SOCKET) return;
#ifdef _WIN32
...
inline int64_t timeout_ms(int seconds) {
...
inline TimeoutChoice connection_timeout_for(bool streaming,
...
inline OriginParts parse_origin(const std::string &origin) {
    OriginParts out;
...
inline int64_t deadline_after(int64_t started_ms, int64_t delay_ms) {
...
inline bool numeric_host(const std::string &host) {
    struct in_addr ipv4{};
